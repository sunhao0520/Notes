...
file_name = input("请输入文件后辍数字-订单列表")
convernt_id = input("xls文件按1，xlsx文件不按")

# 转格式 xls 转 xlsx
if convernt_id == "1":
    fname = "C:\\Users\\94960\\Desktop\\python执行\\订单列表 (" + file_name + ").xls"
    ...
    os.remove(fname)
    print("已转变格式为xlsx，并删除源文件")
    excel.Application.Quit()
else :
    print("没有转文件格式\n")

wb = load_workbook("C:/Users/94960/Desktop/python执行/订单列表 (" + file_name + ").xlsx")
# 订单列表 (2).xlsx
# wb = load_workbook("C:/Users/94960/Desktop/python执行/订单列表 (4).xlsx")

sheet = wb.active  # 执行默认的第一个活动表
# sheet=wb["列表1"]  #执行列表1

courseDate = {}  # 提前定义我需要的字典
i = 0  # 看计算次数的
...
print("表格中含有的时间有%s个;" % length_jianzhidui)  # 键值对数量数量个数
# 输出数据到新的python文件里
print("writing results...已写入py文件")
...
for key_words in courseDate.keys():  # 遍历日期
    # print(key_words) #输出遍历出来的日期
    for key in courseDate[key_words].keys():  # 遍历课程
        # print(key)   #输出遍历出来的课程名
